...
// NewProtocolManager returns a new datx sub protocol manager. The DATx sub protocol manages peers capable
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing DATx peer", "peer", id)

	// Unregister the peer from the downloader and DATx peer set
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	// Execute the DATx handshake
	td, head, genesis := pm.blockchain.Status()
	if err := p.Handshake(pm.networkId, td, head, genesis); err != nil {
		p.Log().Debug("DATx handshake failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("DATx message handling failed", "err", err)
			return err
		}
	}
}
...
// EthNodeInfo represents a short summary of the DATx sub-protocol metadata known
// about the host peer.
type EthNodeInfo struct {
	Network    uint64      `json:"network"`    // DATx network ID (1=Frontier, 2=Morden, Ropsten=3)
