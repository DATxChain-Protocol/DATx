...
#
# The cluster contains a bootnode, a gdatx node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the gdatx and
# swarm nodes
start_bootnode() {
...
# start_gdatx_node starts a gdatx node with --datadir pointing at <base-dir>/gdatx
# and a single, unlocked account with password "gdatx"
start_gdatx_node() {
  local dir="${base_dir}/gdatx"
  mkdir -p "${dir}"

  local password="gdatx"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating gdatx account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gdatx account address"
  fi
  ...
  start_node "gdatx" "${GETH_IP}" "$(which gdatx)" ${args[@]}
}
...
create_account() {
  ...
  gdatx --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
