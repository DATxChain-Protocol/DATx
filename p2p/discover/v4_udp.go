...
var (
	datxPrefix     = []byte("discovery v4 for DATx protocol")
	datxPrefixSize = len(datxPrefix)
	macSize        = 256 / 8
	sigSize        = 520 / 8

	headSize = datxPrefixSize + macSize + sigSize // space of packet frame data
)
...
func (t *UDPv4) encode(priv *ecdsa.PrivateKey, req packetV4) (packet, hash []byte, err error) {
	...
	copy(packet, datxPrefix)
	copy(packet[datxPrefixSize+macSize:], sig)
	// add the hash to the front. Note: this doesn't protect the
	// packet in any way. Our public key will be part of this hash in
	// The future.
	hash = crypto.Keccak256(packet[datxPrefixSize+macSize:])
	copy(packet[datxPrefixSize:], hash)
...
func decodeV4(buf []byte) (packetV4, encPubkey, []byte, error) {
	...
	prefix, hash, sig, sigdata := buf[:datxPrefixSize], buf[datxPrefixSize:datxPrefixSize+macSize], buf[datxPrefixSize+macSize:headSize], buf[headSize:]
	shouldhash := crypto.Keccak256(buf[datxPrefixSize+macSize:])
	if !bytes.Equal(hash, shouldhash) {
		return nil, encPubkey{}, nil, errBadHash
	}
	if !bytes.Equal(prefix, datxPrefix) {
		return nil, encPubkey{}, nil, errBadPrefix
	}
