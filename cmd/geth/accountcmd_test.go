...
func TestAccountListEmpty(t *testing.T) {
	gdatx := runGdatx(t, "account", "list")
	gdatx.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gdatx := runGdatx(t, "account", "list", "--datadir", datadir)
	defer gdatx.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gdatx := runGdatx(t, "account", "new", "--lightkdf")
	defer gdatx.ExpectExit()
	gdatx.Expect(`
...
`)
	gdatx.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gdatx := runGdatx(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	gdatx := runGdatx(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gdatx.ExpectExit()
	gdatx.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gdatx.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gdatx.StderrText(), m) {
...
// https://github.com/KunkaYU/go-DATx/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gdatx.SetTemplateFunc("keypath", func(file string) string {
