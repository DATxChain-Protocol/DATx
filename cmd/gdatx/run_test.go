...
func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "gdatx-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "gdatx-test" in runGdatx.
	reexec.Register("gdatx-test", func() {
...
// spawns gdatx with the given command line args. If the args don't set --datadir, the
// child g gets a temporary data directory.
func runGdatx(t *testing.T, args ...string) *testgdatx {
	tt := &testgdatx{}
	...
	// Boot "gdatx". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("gdatx-test", args...)
