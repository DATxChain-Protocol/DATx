...
// faucet is a DATX faucet backed by a light client.
package main
...
var (
	...
	statsFlag   = flag.String("datxstats", "", "Ethstats network monitoring auth string")
...
var (
	datx = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
type faucet struct {
	...
	client *datxclient.Client  // Client connection to the Ethereum chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the datxstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightEthereum
			ctx.Service(&serv)
			return datxstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := datxclient.NewClient(api)
...
// apiHandler handles requests for DATX grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, datx),
		"funded":   nonce,
	...
	for {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), datx)
...
func (f *faucet) loop() {
	...
	for {
		...
		case head := <-heads:
			...
			balance = new(big.Int).Div(balance, datx)
