...
func NewPrivateAdminAPI(datx *Ethereum) *PrivateAdminAPI {
...
func NewPublicDebugAPI(datx *Ethereum) *PublicDebugAPI {
	return &PublicDebugAPI{datx: datx}
}
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	if blockNr == rpc.PendingBlockNumber {
		...
		_, stateDb := api.datx.miner.Pending()
	...
	stateDb, err := api.datx.BlockChain().StateAt(block.Root())
	if err != nil {
...
func NewPrivateDebugAPI(config *params.ChainConfig, datx *Ethereum) *PrivateDebugAPI {
	return &PrivateDebugAPI{config: config, datx: datx}
}
...
func (api *PrivateDebugAPI) traceBlock(block *types.Block, logConfig *vm.LogConfig) (bool, []vm.StructLog, error) {
	...
	if err := api.datx.engine.VerifyHeader(blockchain, block.Header(), true); err != nil {
...
func (api *PrivateDebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	db := core.PreimageTable(api.datx.ChainDb())
...
func (api *PrivateDebugAPI) GetBadBlocks(ctx context.Context) ([]core.BadBlockArgs, error) {
	return api.datx.BlockChain().BadBlocks()
}
...
func (api *PrivateDebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]common.Address, error) {
	...
	startBlock = api.datx.blockchain.GetBlockByNumber(startNum)
	if startBlock == nil {
	...
	if endNum == nil {
		endBlock = startBlock
		startBlock = api.datx.blockchain.GetBlockByHash(startBlock.ParentHash())
		if startBlock == nil {
			return nil, fmt.Errorf("block %x has no parent", endBlock.Number())
		}
	} else {
		endBlock = api.datx.blockchain.GetBlockByNumber(*endNum)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByHash(startHash common.Hash, endHash *common.Hash) ([]common.Address, error) {
	var startBlock, endBlock *types.Block
	startBlock = api.datx.blockchain.GetBlockByHash(startHash)
	if startBlock == nil {
	...
	if endHash == nil {
		...
		endBlock = api.datx.blockchain.GetBlockByHash(*endHash)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]common.Address, error) {
	...
	oldTrie, err := trie.NewSecure(startBlock.Root(), api.datx.chainDb, 0)
	if err != nil {
		return nil, err
	}
	newTrie, err := trie.NewSecure(endBlock.Root(), api.datx.chainDb, 0)
	if err != nil {
		return nil, err
	}
