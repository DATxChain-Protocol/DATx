...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gdatx.LightScryptN, Gdatx.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.datx", new NodeConfig());
			node.start();
			...
			DATxClient ec = node.getDATxClient();
	...
	// signatures too. Regression test for go-datx issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogdatxer
	...
	workspace, err := ioutil.TempDir("", "gdatx-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/datx/gdatxtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gdatx and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.datx", "github.com/KunkaYU/go-DATx/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gdatx.aar"), "gdatx.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "datx", "gdatxtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
