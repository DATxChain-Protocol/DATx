...
import (
	...
	"github.com/DATxChain-Protocol/DATx/accounts/usbwallet/internal/trezor"
...
// or whether the DATx app was not started on it.
...
// DATx address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.DATxAddress)
	if _, err := w.trezorExchange(&trezor.DATxGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.DATxTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.DATxTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
