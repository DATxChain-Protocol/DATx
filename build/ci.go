...
import (
	...
	"github.com/KunkaYU/go-DATx/internal/build"
)

var (
	// Files that end up in the gdatx*.zip archive.
	gdatxArchiveFiles = []string{
		"COPYING",
		executablePath("gdatx"),
	}

	// Files that end up in the gdatx-alltools*.zip archive.
...
func doLint(cmdline []string) {
	...
	// Run fast linters batched togdatxer
...
func doArchive(cmdline []string) {
	var (
		...
		upload = flag.String("upload", "", `Destination to upload the archives (usually "gdatxstore/builds")`)
		ext    string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		...
		gdatx     = "gdatx-" + base + ext
		alltools = "gdatx-alltools-" + base + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(gdatx, gdatxArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gdatx, alltools} {
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gdatx-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gdatx.exe" {
			gdatxTool = file
		} else {
	...
	// first section contains the gdatx binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Gdatx":     gdatxTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gdatx.nsi", filepath.Join(*workdir, "gdatx.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("gdatx-" + archiveBasename(*arch, env) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gdatx.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload = flag.String("upload", "", `Destination to upload the archive (usually "gdatxstore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	...
	build.MustRun(gomobileTool("bind", "--target", "android", "--javapkg", "org.datx", "-v", "github.com/KunkaYU/go-DATx/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("gdatx.aar", filepath.Join(GOBIN, "gdatx.aar"))
		return
	}
	...
	archive := "gdatx-" + archiveBasename("android", env) + ".aar"
	os.Rename("gdatx.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "--target", "ios", "--tags", "ios", "-v", "github.com/KunkaYU/go-DATx/mobile")
	...
	archive := "gdatx-" + archiveBasename("ios", env)
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "Gdatx.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "Gdatx.podspec", "--allow-warnings", "--verbose")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "gdatxstore/builds")`)
